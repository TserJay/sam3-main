...
def download_ckpt_from_hf():
    # config.json 和 sam3.pt 都在 /pengyi-sam3/ 目录下）
    SAM3_MODEL_ID = "facebook/sam3"  # 保留原有变量，避免其他代码报错
    SAM3_CKPT_NAME = "sam3.pt"      # 保留原有变量
    SAM3_CFG_NAME = "config.json"    # 保留原有变量
    
    local_cfg_path = "config.json"
    local_ckpt_path = "sam3.pt"
    
    if not os.path.exists(local_cfg_path):
        raise FileNotFoundError(f"本地配置文件不存在！")
    if not os.path.exists(local_ckpt_path):
        raise FileNotFoundError(f"本地模型文件不存在！")
